...
class TestRAGSystem(unittest.TestCase):
    ...
    def test_query_without_session(self):
        ...
        # Setup mocks
        self.rag_system.ai_generator.generate_response.return_value = "AI response about Python"
        self.rag_system.tool_manager.get_tool_definitions.return_value = [{"name": "search_course_content"}]
        ...
        self.assertEqual(call_args.kwargs["query"], "Answer this question about course materials: What is Python?")
        ...
        self.assertEqual(sources, [{"text": "Python Basics - Lesson 1", "link": "https://example.com/lesson1"}])
    ...
    def test_query_with_session(self):
        ...
        # Setup mocks
        self.rag_system.session_manager.get_conversation_history.return_value = "Previous conversation context"
        self.rag_system.ai_generator.generate_response.return_value = "Contextual AI response"
        ...
        call_args = self.rag_system.ai_generator.generate_response.call_args
        self.assertEqual(call_args.kwargs["conversation_history"], "Previous conversation context")
    ...
    def test_query_with_tool_execution(self):
        ...
        self.rag_system.ai_generator.generate_response.return_value = "Based on my search, Python is a programming language."
        self.rag_system.tool_manager.get_tool_definitions.return_value = [
            {
        ...
        self.rag_system.tool_manager.get_last_sources.return_value = [
            {"text": "Python Programming - Lesson 2", "link": "https://example.com/python2"},
            {"text": "Advanced Python - Lesson 1", "link": "https://example.com/advanced1"}
        ]
    ...
    def test_query_prompt_formatting(self):
        ...
        for query in test_queries:
            self.rag_system.ai_generator.generate_response.return_value = "Test response"
            self.rag_system.tool_manager.get_tool_definitions.return_value = []
            self.rag_system.tool_manager.get_last_sources.return_value = []
    ...
    def test_empty_sources_handling(self):
        ...
        self.rag_system.ai_generator.generate_response.return_value = "General knowledge response"
        self.rag_system.tool_manager.get_tool_definitions.return_value = []
        self.rag_system.tool_manager.get_last_sources.return_value = []
    ...
    def test_add_course_document_success(self):
        ...
        mock_chunks = [
            CourseChunk(content="Chunk 1", course_title="Test Course", lesson_number=1, chunk_index=0),
            CourseChunk(content="Chunk 2", course_title="Test Course", lesson_number=1, chunk_index=1)
        ]
        ...
        self.rag_system.document_processor.process_course_document.assert_called_once_with("/path/to/course.txt")
        
        # Verify vector store operations
        self.rag_system.vector_store.add_course_metadata.assert_called_once_with(mock_course)
        self.rag_system.vector_store.add_course_content.assert_called_once_with(mock_chunks)
    ...
    def test_add_course_folder_with_clear_existing(self):
        ...
        with patch('os.path.exists', return_value=True), \
             patch('os.listdir', return_value=['course1.txt', 'course2.pdf', 'ignored.log']), \
             patch('os.path.isfile', return_value=True):
            
            # Setup mock responses
            mock_course1 = Course(title="Course 1", lessons=[])
            mock_course2 = Course(title="Course 2", lessons=[])
            mock_chunks1 = [CourseChunk(content="Chunk", course_title="Course 1", chunk_index=0)]
            mock_chunks2 = [CourseChunk(content="Chunk", course_title="Course 2", chunk_index=0)]
            ...
            courses, chunks = self.rag_system.add_course_folder("/path/to/courses", clear_existing=True)
            
            # Verify data was cleared
            self.rag_system.vector_store.clear_all_data.assert_called_once()
            
            # Should process 2 files (txt and pdf, not log)
            self.assertEqual(self.rag_system.document_processor.process_course_document.call_count, 2)
    ...
    def test_add_course_folder_skip_existing(self):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            self.rag_system.document_processor.process_course_document.side_effect = [
                (existing_course, []),
                (new_course, [CourseChunk(content="New", course_title="New Course", chunk_index=0)])
            ]
            self.rag_system.vector_store.get_existing_course_titles.return_value = ["Existing Course"]
    ...
    def test_get_course_analytics(self):
        ...
        self.rag_system.vector_store.get_existing_course_titles.return_value = ["Course A", "Course B", "Course C"]
    ...
    def test_integration_query_flow(self):
        ...
        self.rag_system.session_manager.get_conversation_history.return_value = "User: Hi\nAssistant: Hello!"
        self.rag_system.tool_manager.get_tool_definitions.return_value = [
            {"name": "search_course_content", "description": "Search courses"}
        ]
        self.rag_system.ai_generator.generate_response.return_value = "Python is a versatile programming language."
        self.rag_system.tool_manager.get_last_sources.return_value = [
            {"text": "Python Fundamentals", "link": "https://course.com/python"}
        ]
        
        # Execute complete flow
        response, sources = self.rag_system.query("What is Python?", session_id=session_id)
        ...
        self.assertEqual(ai_call_args.kwargs["query"], "Answer this question about course materials: What is Python?")
        self.assertEqual(ai_call_args.kwargs["conversation_history"], "User: Hi\nAssistant: Hello!")
        self.assertEqual(ai_call_args.kwargs["tools"], [{"name": "search_course_content", "description": "Search courses"}])
        self.assertEqual(ai_call_args.kwargs["tool_manager"], self.rag_system.tool_manager)
        ...
        self.assertEqual(sources, [{"text": "Python Fundamentals", "link": "https://course.com/python"}])
